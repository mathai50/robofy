...
interface CircularProgressBarProps {
  ...
  size?: 'sm' | 'md' | 'lg';
  /**
   * Optional label to display below the progress bar
   */
  label?: string;
  /**
...
const CircularProgressBar = React.forwardRef<HTMLDivElement, CircularProgressBarProps>(
  (
    {
      value,
      color = 'primary',
    ...
    const colorClasses = {
      primary: 'text-primary-accent-1',
      success: 'text-primary-status-success',
      warning: 'text-primary-status-warning',
      error: 'text-primary-status-error',
      neutral: 'text-primary-neutral',
    };
      ...
      <div className={`flex flex-col items-center ${className}`} ref={ref}>
        ...
        <div className="mt-3 text-center">
          <div className="flex items-center justify-center gap-1">
            {label && (
              <h3 className={`font-semibold text-gray-800 ${textSizeClasses[size]}`}>
                {label}
              </h3>
            )}
            {tooltip && (
